...
name: loop-triage
description: >-
  Agentic Loop Engineering Kit Phase 0–0b — context sync, wiki/Jira/Confluence, graphify, ticket ack.
...
- [ ] Profile `pattern_doc` if set → read (active repo subsection when multi-repo)
- [ ] Profile `loop_playbook` if set → read
- [ ] Profile `context_skill` if set (e.g. `cell-health-mvp` Phase 0)
- [ ] Jira / Confluence per profile
- [ ] `graphify query` for touched repos
